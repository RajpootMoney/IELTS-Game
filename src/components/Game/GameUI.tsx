    ...
    <>
      <div className="absolute top-0 left-0 right-0 flex justify-between items-start p-4 z-20 pointer-events-none">
        <div className="flex flex-col">
          ...
          <span className="font-pixel text-[10px] md:text-xs text-gray-500 mt-1">
            {modeInfo.icon} {modeInfo.title.toUpperCase()}
          </span>
        </div>
            ...
            <>
              ...
              <span
                className={`font-pixel text-xl md:text-2xl neon-text ${
                  timeLeft <= 15 ? 'text-neon-red animate-pulse-glow' : 'text-neon-yellow'
                }`}
              >
                {timeLeft}s
              </span>
            ...
            {[...Array(maxLives)].map((_, i) => (
              <span
                ...
                className={`font-pixel text-xl md:text-2xl ${
                  i < lives ? 'text-neon-pink neon-text' : 'text-gray-600'
                }`}
              >
                ♥
              </span>
